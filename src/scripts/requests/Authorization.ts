...
async function RegisterToken(request :Request): Promise<any>
{
	const response = await fetch(request).then((response): Promise<any> =>
	{
		if (response.ok)
...
async function ValidateToken(request: Request):Promise<User>
{
	const response = await fetch(request).then((response): Promise<any> => 
	{
		if (response.ok)
...
async function DeleteToken(request: Request): Promise<boolean | void>
{
