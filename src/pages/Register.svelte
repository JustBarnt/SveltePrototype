<script lang="ts">
	...
	const AwaitRegistration = async (awaited: any): Promise<any> => { return await awaited; };
	const alertCss: IStyles = { position: "relative", bottom: "clamp(0px, 2.5rem, 3.5rem)", background: AwaitRegistration(isSuccessful) ? Colors.GREEN : Colors.RED };
	...
	*/
	function Register(event: any): boolean | Promise<boolean>
	{
		...
		{
			if( field.length === 0 || field === null ) emptyFields[index] = false;
...
<section>
	<form
	method="POST"
	on:submit|preventDefault={Register} autocomplete="off">
		<h2>Create New Account</h2>

		{#await AwaitRegistration(isSuccessful) then PromiseResults}
			<Alert visible={PromiseResults === null ? AlertVisibility.Hidden : AlertVisibility.Visible} message={alertMessage} styles={alertCss} />
		{/await}
		
		<label for="firstName">First Name: *</label> 
		<p class="required-field" style={`display: ${emptyFields[0] === false ? "inherit" : "none"}`}>Please fill out the required field</p>
		<input type="text" name="firstName" value="" />
		
		<label for="lastName">Last Name: *</label>
		<p class="required-field" style={`display: ${emptyFields[1] === false ? "inherit" : "none"}`}>Please fill out the required field</p>
		<input type="text" name="lastName" value="" />

		<label for="username">Username: *</label>
		<p class="required-field" style={`display: ${emptyFields[2] === false ? "inherit" : "none"}`}>Please fill out the required field</p>
		<input type="text" name="username" value="" />

		<label for="password">Password: *</label>
		<p class="required-field" style={`display: ${emptyFields[3] === false ? "inherit" : "none"}`}>Please fill out the required field</p>
		<input type="password" name="password" value="" /> 

		<input type="submit" value="Submit" />
		<p class="login" on:click={() => Navigation.ChangePage(Navigation.Page.Login)}>Login</p>
	</form>
</section>
